...
function TrialDetail({ trial, index }: { trial: TrialResult; index: number }) {
  ...
  return <div className="trial-detail" key={index}>
    ...
    {trial.route === "vercel-ai-gateway" && <p className="muted">接続経路: Vercel AI Gateway。判断時間は中継を含みます。モデル名のエイリアスから上流の版は推定しません。</p>}
    {trial.route === "openrouter" && <p className="muted">接続経路: OpenRouter。判断時間は中継を含みます。版情報は応答に含まれる場合のみ記録しています。</p>}
    <div className="trial-grid">
